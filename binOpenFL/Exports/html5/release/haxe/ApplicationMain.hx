...
@:dox(hide) class ApplicationMain {
	...
	public static function main () {
		...
		var config = {
			...
			name: "MainOpenFL",
			orientation: "",
			packageName: "net.nanjizal.justDrawing",
			version: "1.0.0",
			windows: [
	...
	macro public static function getEntryPoint () {
		...
		switch (Context.follow (Context.getType ("testjustDrawing.MainFlash"))) {
			...
			case TInst (t, params):
				...
				if (hasMain) {
					...
					return Context.parse ("@:privateAccess testjustDrawing.MainFlash.main ()", Context.currentPos ());
					...
					Context.fatalError ("Main class \"testjustDrawing.MainFlash\" has neither a static main nor a constructor.", Context.currentPos ());
					
				}
				
			default:
				
				Context.fatalError ("Main class \"testjustDrawing.MainFlash\" isn't a class.", Context.currentPos ());
...
@:keep @:dox(hide) class DocumentClass extends testjustDrawing.MainFlash {}
